...
public class SeedMapperImpl implements SeedMapper {
	...
	/**
	 * 验证该种子是否已经派发过 内部逻辑： 1：找最大索引Id行，获取种子value值 为V（有记录行） 2：若没有记录行，则返回false （无记录行） 2：若value>V,则返回false，否则返回true
	 * 
	 * @param value
	 *            种子值
	 * @return 是否派发过
	...
	/**
	 * 找到种子标示值 内部逻辑： 1：找最大索引Id行，获取种子value值 为V 1.1返回记录行（有记录） 1.2返回null （无记录）
	 * 
	 * @return 种子标识
	 * @throws Exception
	 *             持久层异常
