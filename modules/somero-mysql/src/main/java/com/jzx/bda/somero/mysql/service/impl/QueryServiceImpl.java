...
import com.jzx.bda.somero.mysql.util.BeanUtils;
...
public class QueryServiceImpl implements QueryService {
	...
	@Override
	public List<Object> getTracesByDuration(TraceData traceData) {
		Map paramMap = BeanUtils.getFieldValueMap(traceData);
		List<Trace> list = traceMapper.queryTracesByDuration(paramMap);
		List<Object> array = new ArrayList<Object>();
		for (Trace trace : list) {
			Map map = new HashMap();
			map.put("serviceId", trace.getService());
			map.put("timestamp", trace.getTime());
	...
	@Override
	public int getTracesByDurationCount(TraceData traceData) {
		Map paramMap = BeanUtils.getFieldValueMap(traceData);
		int count = traceMapper.queryTracesByDurationCount(paramMap);
		return count;
	}
}
