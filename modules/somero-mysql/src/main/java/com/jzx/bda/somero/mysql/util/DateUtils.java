...
public class DateUtils {
	...
	public static Date transferDate(String source) throws Exception {
		...
		return parse(str, DateUtils.yyyy_MM_dd_EN);
	}
	...
	public static String dateToCalendarString(Calendar date, String pattern) {
		DateFormat df = new SimpleDateFormat(pattern);// 设置你想要的格式
	...
	public static String getGTMDateStr(Date date, String formatStr) {
		DateFormat sdf = new SimpleDateFormat(formatStr, Locale.US);
	...
	// String lastDay = DateHelper.format(getLastMonthLastDay(), "yyyy-MM-dd") +
	// " 23:59:59";
	// String lastMonth = getDateOfDay(DateHelper.currentDate(), -30,
	// "yyyy-MM-dd") + " 00:00:00";
	// long lastDayTime = DateHelper.parse(lastDay,
	// DateHelper.yyyy_MM_dd_HH_mm_ss_EN).getTime();
	// long lastMonthTime = DateHelper.parse(lastMonth,
	// DateHelper.yyyy_MM_dd_HH_mm_ss_EN).getTime();
	// if (lastMonthTime > lastDayTime) {
	...
	public static void main(String[] args) {
		Date date = new Date(1463018090942L);
		System.out.println(DateUtils.format(date, DateUtils.YYYY_MM_DD_HH_MM_SS_SSS));
	}
}
